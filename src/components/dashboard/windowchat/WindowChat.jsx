...
const HeaderChat = ({ focus }) => {
	...
	const topicons = [
		...
		{
			icon: (
				<div onClick={() => setShowWindow(false)}>
					<ICanCel active={focus} />
				</div>
			),
	...
	const IconButton = ({ icon }) => {
		return (
			<Tooltip lable={icon.title} position={'top'}>
				<div className='w-[24px] h-[24px] hover:bg-white/10 rounded-full cursor-pointer '>{icon.icon}</div>
			</Tooltip>
		);
	};
			...
			<Tooltip lable={'Cài đặt chat'} position='top'>
				<div className='flex flex-row items-center space-x-2 relative group'>
					...
					<div className='flex flex-col space-y-1 '>
						<span className='text-primaryText font-semibold leading-3 text-[15px]'>Nguyễn Tiến Thành</span>
					...
					<div
						className='absolute top-0 left-0 w-[calc(100%+10px)] h-11 group-hover:bg-white/10 rounded-md -translate-x-4 
          -translate-y-[6px] cursor-pointer'
					></div>
				</div>
			</Tooltip>
		...
		<div className='flex h-[60px]  flex-row items-center py-3'>
			...
			<div className='w-[148px] bg-commentBackground h-9 flex flex-none items-center rounded-full ml-1'>
				<div className='w-full flex flex-row justify-between items-center my-2 mx-3'>
					<div
						className='w-[104px] text-[15px] text-secondaryText outline-none badge'
