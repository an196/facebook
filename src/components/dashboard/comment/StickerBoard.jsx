...
const TabBtn = ({ tab, onClick }) => {
	return (
		<div className={`${tab.filter && 'filter-icon'} w-[39px] flex justify-center items-center hover:bg-white/20`}
...
function StickerBoard() {
	...
	const tabs = [
		...
		{ name: 'history', icon: <ClockCircle />, element: <StickerTab stickers={historyStickers} />, filter: true, },
		{ name: 'avatar', icon: <MotionViolet />, element: <StickerTab stickers={avatarStickers} />, filter: false, stickers: avatarStickers },
		{ name: 'bee', icon: <QooBeeAgapi />, element: <StickerTab stickers={beeStickers} isDynamic />, filter: false },
		{ name: 'dove', icon: <DoveCity />, element: <StickerTab stickers={doveStickers} isDynamic />, filter: false, stickers: doveStickers },
	...
	const hdlTabClick = async (name) => {
		if (name !== currentTab.name) {
			...
			const changeTab = (ms) => new Promise((res) => setTimeout(() => setCurrentTab(state => state = newTab), ms));
			setCurrentTab(null);
			await changeTab(0);
		...
		<>
			...
			bg-cardBackground rounded-lg drop-shadow-[0_0_6px_rgba(0,0,0,0.2)]'>
				<div className='h-[39px] flex flex-row'>
					{
						tabs.map((tab, idx) => (<TabBtn tab={tab} key={idx} onClick={hdlTabClick} />))
					}
				</div>
