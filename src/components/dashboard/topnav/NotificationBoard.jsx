	...
	return (
		<div className='w-full flex flex-row justify-between space-x-3 px-4 items-stretch relative p-2'>
			<div className='w-[56px] h-[56px]  flex-none relative'>
				<img src={notification.img} className='w-[56px] h-[56px] rounded-full'></img>
				<div
					className={`absolute bottom-0 right-0 z-20  ${
						notification.type === typeNotificaiton.group ? 'icon-group-blue' : 'icon-message-news'
					}`}
				></div>
			</div>
			<div className='text-[#e4e6eb] text-[15px] font-normal grow flex flex-col  flex-wrap overflow-hidden'>
				...
				<span
					className={`text-[13px] font-semibold ${
						notification.status === status.unseen ? 'text-[#2e89ff]' : 'text-secondaryText'
					}`}
				>
					{notification.time}
				</span>
			...
			<div className='flex items-center justify-center'>
				<div
					className={`w-3 h-3 rounded-full bg-[#2e89ff]  ${
						notification.status === status.unseen ? 'opacity-100' : 'opacity-0'
					}`}
				></div>
			</div>
...
function NotificationBoard() {
	...
	return (
		<div
			className='absolute right-0 top-10 w-[360px] max-h-[801px] bg-[#242526] text-[#e4e6eb] rounded-md'
			ref={notifiBoard}
		>
				...
				<div className='flex flex-col overflow-y-auto max-h-[801px] pb-4 scroll-smooth'>
					...
					<div className='h-[52px] flex items-center px-4 py-2'>
						<span className='h-9 flex flex-row items-center space-x-2 text-[15px] font-semibold'>
							<span
								className='align-middle px-3 flex items-center bg-[#2d88ff]/30 hover:bg-[#2d88ff]/20 h-9 rounded-full 
                  				text-[#2d88ff]'
							>
								Tất cả
							</span>
